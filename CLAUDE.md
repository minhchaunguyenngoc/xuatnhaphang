...
- Sau mỗi lần xuất, `products.import_price` được **tính lại** từ các lô còn tồn (`recompute_avg_cost`). **Hết lô thì GIỮ NGUYÊN giá vốn lần cuối, không đưa về 0** — cột "Giá nhập" tụt về 0 ngay khi bán hết bị đọc nhầm thành mất dữ liệu; định giá kho vẫn đúng vì tồn = 0. `backfill_last_known_cost` (SCHEMA_VERSION 3) khôi phục giá cho CSDL cũ đã bị ghi 0. Khi còn tồn kho, `import_price` là giá vốn hệ thống quản lý và không cho sửa tay (UI khóa ô + backend giữ giá cũ trong `update_product`); tồn = 0 thì mở cho sửa tay.
...
- Báo cáo lợi nhuận có 2 cách gom, **tổng phải khớp nhau** trong cùng một kỳ: `get_profit_report` (theo sản phẩm) và `get_receipt_profit_report` (theo hoá đơn, lọc được theo khách; hàng trả trừ vào chính hoá đơn gốc bất kể ngày lập phiếu trả).
